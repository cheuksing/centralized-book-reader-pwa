...
for (const target of ['http://example.com', 'https://user:pass@example.com', 'https://example.com:444', 'https://localhost/a', 'https://169.254.169.254/a', 'https://[::1]/a', 'https://192.168.1.1/a']) {
  let rejected = false
  try { validateTargetUrl(target) } catch { rejected = true }
...
assert(response.headers.get('etag') === 'v1' && (await response.json() as { ok: boolean }).ok, 'ArrayBuffer responses must preserve allowlisted headers and body')
