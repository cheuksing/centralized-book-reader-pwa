...
const SAFE_RESPONSE_HEADERS = ['Content-Type', 'Content-Length', 'ETag', 'Last-Modified'];
export default {
...
async function proxy(target, origin) {
    let current = target;
...
function isDisallowedIpLiteral(hostname) {
    ...
    if ((first & 0xff00) === 0xff00)
        return true;
    if ((first === 0xfec0) || (first >= 0xfec0 && first <= 0xfeff))
        return true;
...
function jsonError(status, code, origin, extraHeaders) {
...
function setCorsHeaders(headers, origin, preflight = false) {
    if (origin === null)
        return;
