...
export type UserScriptStatus =
  ...
  | { kind: 'missing' }
  | { kind: 'outdated'; scriptVersion?: string }
  | { kind: 'unsupported' }
  | { kind: 'permission-required' }

export type BridgeMessage = {
  channel: typeof USER_SCRIPT_CHANNEL
  protocol: number
  type: 'probe' | 'probe-result' | 'request' | 'response' | 'error' | 'cancel' | 'access' | 'access-result'
  requestId: string
  url?: string
  accept?: string
  ...
  body?: ArrayBuffer
  code?: 'request-permission-denied' | 'request-unavailable' | 'request-cancelled' | 'target-rejected'
  access?: 'granted' | 'denied' | 'unavailable'
}

const messageTypes = new Set<BridgeMessage['type']>(['probe', 'probe-result', 'request', 'response', 'error', 'cancel', 'access', 'access-result'])
...
export function isCompatibleProtocol(message: Pick<BridgeMessage, 'protocol'>): boolean { return message.protocol === USER_SCRIPT_PROTOCOL }

export function matchesBridgeResponse(event: { source: unknown; origin: string; data: unknown }, source: unknown, origin: string, requestId: string): event is { source: unknown; origin: string; data: BridgeMessage } {
  return event.source === source && event.origin === origin && isBridgeMessage(event.data) && event.data.requestId === requestId
}

export function bridgeFailureKind(code: BridgeMessage['code'] | 'timeout'): 'aborted' | 'offline' | 'request-permission-denied' | 'target-rejected' | 'user-script-unavailable' {
  if (code === 'request-permission-denied') return 'request-permission-denied'
  if (code === 'target-rejected') return 'target-rejected'
  if (code === 'request-cancelled') return 'aborted'
  if (code === 'timeout') return 'user-script-unavailable'
  return 'offline'
}
...
export function validateTargetUrl(value: string): URL {
  let target: URL
  try { target = new URL(value) } catch { throw new Error('Remote targets must be absolute HTTPS URLs.') }
  ...
  if (authority.includes('@')) throw new Error('Remote targets must be credential-free HTTPS URLs.')
  const hostname = normalizeHostname(target.hostname)
  if (!hostname || isBlockedHostname(hostname) || isDisallowedIpLiteral(hostname)) throw new Error('Remote target host is not allowed.')
  return target
}

export function responseFromBridge(message: BridgeMessage): Response {
  if (message.type !== 'response' || typeof message.status !== 'number' || !(message.body instanceof ArrayBuffer)) throw new Error('The userscript returned an invalid response.')
  const headers = new Headers()
  for (const name of ['content-type', 'content-length', 'etag', 'last-modified']) {
    const value = message.headers?.[name]
    if (typeof value === 'string' && value.length <= 2_000 && (name !== 'content-length' || /^\d+$/.test(value))) headers.set(name, value)
  }
