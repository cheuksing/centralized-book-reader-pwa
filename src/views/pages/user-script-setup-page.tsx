...
export function UserScriptSetupPage() {
  ...
  return <main className="setup-page">
    ...
    <section aria-busy={busy} className="setup-card">
      ...
      {(model.userScriptStatus.kind === 'ready' || model.userScriptStatus.kind === 'permission-required') && <button className="secondary-button" disabled={busy} onClick={() => { void model.grantUserScriptAccess() }} type="button">Grant remote-request access</button>}
    ...
    {model.userScriptStatus.kind === 'permission-required' && <p className="setup-note">If a browser remembers a rejected grant, Bookshelf cannot override it. Open the Violentmonkey Dashboard, select <strong>Bookshelf CORS Bridge</strong>, review or allow its site or host access in browser extension settings, then return and select Check again. You can also <a href={model.userScriptUrl}>reinstall or update the userscript</a>.</p>}
